  ...
  return (
    <div className="chat-container">
      <div className="chat-header">
        <div>
          <Text strong className="chat-title">{chat.title}</Text>
          <br />
          <Text type="secondary" className="chat-subtitle">参与人：你 / 业务助手</Text>
        </div>
      ...
      <div className="input-area">
        ...
        />
        <Button type="primary" onClick={() => handleSend()} loading={loading} disabled={pendingSlots.length > 0} className="send-btn">
          发送
        </Button>
      </div>
      {chat.messages.length > 0 && chat.messages[chat.messages.length-1].citations && (
        <KnowledgeCitation citations={chat.messages[chat.messages.length-1].citations} />
      )}
    </div>
  );
} 